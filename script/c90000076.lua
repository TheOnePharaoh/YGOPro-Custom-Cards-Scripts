...
function c90000076.initial_effect(c)
	...
	e2:SetOperation(c90000076.operation)
	c:RegisterEffect(e2)
	local e3=e2:Clone()
	...
	e5:SetCost(c90000076.cost)
	e5:SetTarget(c90000076.target)
	e5:SetOperation(c90000076.operation2)
	c:RegisterEffect(e5)
end
function c90000076.filter(c,tp)
	return c:GetSummonPlayer()==tp and c:IsSetCard(0x2d)
end
function c90000076.operation(e,tp,eg,ep,ev,re,r,rp)
	if eg:IsExists(c90000076.filter,1,nil,tp) then
...
function c90000076.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c90000076.filter2,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c90000076.operation2(e,tp,eg,ep,ev,re,r,rp)
	if not e:GetHandler():IsRelateToEffect(e) then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c90000076.filter2,tp,LOCATION_DECK,0,1,1,nil)

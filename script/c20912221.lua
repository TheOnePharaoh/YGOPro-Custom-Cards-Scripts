...
function c20912221.initial_effect(c)
	...
	e1:SetTarget(c20912221.target)
	e1:SetOperation(c20912221.operation)
	c:RegisterEffect(e1)
	--Atk up
...
function c20912221.eqfilter1(c)
	return c:IsFaceup() and c:IsRace(RACE_WARRIOR)
end
function c20912221.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and c20912221.eqfilter1(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c20912221.eqfilter1,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	Duel.SelectTarget(tp,c20912221.eqfilter1,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil)

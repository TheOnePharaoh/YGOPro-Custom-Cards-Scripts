...
function c90000060.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(90000060,0))
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetCost(c90000060.cost)
	e1:SetTarget(c90000060.target)
	e1:SetOperation(c90000060.operation)
	c:RegisterEffect(e1)
	--Special Summon
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(90000060,1))
	e2:SetType(EFFECT_TYPE_ACTIVATE)
	e2:SetCode(EVENT_FREE_CHAIN)
	e2:SetCost(c90000060.cost)
	e2:SetTarget(c90000060.target2)
	e2:SetOperation(c90000060.operation2)
	c:RegisterEffect(e2)
end
function c90000060.cost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c90000060.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return (Duel.CheckLocation(tp,LOCATION_SZONE,6) or Duel.CheckLocation(tp,LOCATION_SZONE,7))
		and Duel.IsExistingMatchingCard(c90000060.filter,tp,LOCATION_DECK+LOCATION_MZONE,0,1,nil) end
end
function c90000060.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c90000060.filter,tp,LOCATION_DECK+LOCATION_MZONE,0,1,1,nil)
	if g:GetCount()>0 then
		Duel.MoveToField(g:GetFirst(),tp,tp,LOCATION_SZONE,POS_FACEUP,true)
	end
end
function c90000060.filter2(c,e,tp)
...
function c90000060.target2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(c90000060.filter2,tp,LOCATION_SZONE+LOCATION_EXTRA,0,1,nil,e,tp) end
...
function c90000060.operation2(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c90000060.filter2,tp,LOCATION_SZONE+LOCATION_EXTRA,0,1,1,nil,e,tp)
	if g:GetCount()>0 then

...
function c90000064.initial_effect(c)
	...
	e3:SetCondition(c90000064.condition)
	e3:SetTarget(c90000064.target)
	e3:SetOperation(c90000064.operation)
	c:RegisterEffect(e3)
end
function c90000064.value(e,c)
	return c:GetCounter(0x1000)*-300
end
function c90000064.condition(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsReason(REASON_DESTROY)
end
function c90000064.filter(c,e,tp)
...
function c90000064.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c90000064.filter,tp,LOCATION_DECK,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_DECK)
end
function c90000064.operation(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c90000064.filter,tp,LOCATION_DECK,0,1,1,nil,e,tp)

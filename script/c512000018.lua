...
function c512000018.initial_effect(c)
	...
	e1:SetTarget(c512000018.eqtg)
	e1:SetOperation(c512000018.eqop)
	c:RegisterEffect(e1)
	--banish
	...
	e2:SetCost(c512000018.rmcost)
	e2:SetTarget(c512000018.rmtg)
	e2:SetOperation(c512000018.rmop)
	c:RegisterEffect(e2)
	if not c512000018.global_check then
		c512000018.global_check=true
		local ge2=Effect.CreateEffect(c)
		ge2:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
		ge2:SetCode(EVENT_ADJUST)
		ge2:SetCountLimit(1)
		ge2:SetProperty(EFFECT_FLAG_NO_TURN_RESET)
		ge2:SetOperation(c512000018.numchk)
		Duel.RegisterEffect(ge2,0)
	end
end
c512000018.xyz_number=80
function c512000018.eqtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c512000018.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	e2:SetValue(c512000018.eqlimit)
	...
	e3:SetCondition(c512000018.btcon)
	e3:SetCost(c512000018.btcost)
	e3:SetOperation(c512000018.btop)
	...
	e4:SetCondition(c512000018.con)
	e4:SetCost(c512000018.btcost)
	e4:SetOperation(c512000018.op)
	e4:SetReset(RESET_EVENT+0x1fe0000)
	c:RegisterEffect(e4)	
end
function c512000018.eqlimit(e,c)
	return c==e:GetLabelObject()
end
function c512000018.rmcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetOverlayCount(tp,1,0)>0 end
	local g=Duel.GetOverlayGroup(tp,1,0)
	Duel.SendtoGrave(g,REASON_COST)
end
function c512000018.rmtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsAbleToRemove,tp,0,LOCATION_GRAVE,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_REMOVE,nil,1,0,0)
end
function c512000018.rmfilter(c,ty)
	return c:IsType(ty) and c:IsAbleToRemove()
end
function c512000018.rmop(e,tp,eg,ep,ev,re,r,rp)
...
function c512000018.btcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetBattleDamage(tp)>=1000
end
function c512000018.btcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	local eq=c:GetEquipTarget()
	if chk==0 then return c:IsDestructable() and eq and eq:IsAbleToGraveAsCost() end
	...
	e1:SetCost(c512000018.spcost)
	e1:SetTarget(c512000018.sptg)
	e1:SetOperation(c512000018.spop)
	e1:SetReset(RESET_EVENT+0x1fe0000)
	c:RegisterEffect(e1)
end
function c512000018.btop(e,tp,eg,ep,ev,re,r,rp)
	local e1=Effect.CreateEffect(e:GetHandler())
	e1:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
	e1:SetCode(EVENT_PRE_BATTLE_DAMAGE)
	e1:SetOperation(c512000018.damop)
	e1:SetReset(RESET_PHASE+PHASE_DAMAGE)
	Duel.RegisterEffect(e1,tp)
end
function c512000018.damop(e,tp,eg,ep,ev,re,r,rp)
	Duel.ChangeBattleDamage(tp,ev/2)
end
function c512000018.con(e,tp,eg,ep,ev,re,r,rp)
...
function c512000018.op(e,tp,eg,ep,ev,re,r,rp)
	...
	e1:SetLabel(cid)
	e1:SetValue(c512000018.refcon)
	e1:SetReset(RESET_CHAIN)
	Duel.RegisterEffect(e1,tp)
end
function c512000018.refcon(e,re,val,r,rp,rc)
	local cc=Duel.GetCurrentChain()
	if cc==0 or bit.band(r,REASON_EFFECT)==0 then return end
	local cid=Duel.GetChainInfo(0,CHAININFO_CHAIN_ID)
	return val/2
end
function c512000018.spcfilter(c)
	return c:IsType(TYPE_MONSTER) and c:IsAbleToGraveAsCost()
end
function c512000018.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c512000018.spcfilter,tp,LOCATION_HAND,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local sg=Duel.SelectMatchingCard(tp,c512000018.spcfilter,tp,LOCATION_HAND,0,1,1,nil)
	Duel.SendtoGrave(sg,REASON_COST)
end
function c512000018.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and c:IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,c,1,0,0)
end
function c512000018.spop(e,tp,eg,ep,ev,re,r,rp)
	if not e:GetHandler():IsRelateToEffect(e) then return end
	Duel.SpecialSummon(e:GetHandler(),0,tp,tp,false,false,POS_FACEUP)
end
function c512000018.numchk(e,tp,eg,ep,ev,re,r,rp)

--ブロックマン
function c48115279.initial_effect(c)
	--check
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
	e1:SetRange(LOCATION_MZONE)
	e1:SetCode(EVENT_PHASE_START+PHASE_DRAW)
	e1:SetCondition(c48115279.regcon)
	e1:SetOperation(c48115279.regop)
	c:RegisterEffect(e1)
	--token
	...
	e2:SetCost(c48115279.spcost)
	e2:SetTarget(c48115279.sptg)
	e2:SetOperation(c48115279.spop)
	c:RegisterEffect(e2)
end
function c48115279.regcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetTurnPlayer()==tp
end
function c48115279.regop(e,tp,eg,ep,ev,re,r,rp)
...
function c48115279.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c48115279.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local ct=e:GetHandler():GetFlagEffectLabel(48115277)
		if not ct then ct=0 end
...
function c48115279.spop(e,tp,eg,ep,ev,re,r,rp)
	local ct=e:GetLabel()

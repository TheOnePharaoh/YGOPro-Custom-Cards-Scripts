...
function c99970023.initial_effect(c)
  ...
  e1:SetTarget(c99970023.natg)
  e1:SetOperation(c99970023.naop)
  c:RegisterEffect(e1)
  --To Hand
...
function c99970023.spfilter(c,e,tp)
  return c:IsCode(99970040) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c99970023.natg(e,tp,eg,ep,ev,re,r,rp,chk)
  if chk==0 then return true end
end
function c99970023.naop(e,tp,eg,ep,ev,re,r,rp)
  local c=e:GetHandler()
  if Duel.NegateAttack() and c:IsRelateToEffect(e) then
  Duel.SendtoGrave(e:GetHandler(),REASON_EFFECT)
  if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
  local g=Duel.GetMatchingGroup(c99970023.spfilter,tp,LOCATION_HAND+LOCATION_DECK,0,nil,e,tp)
...
function c99970023.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
  if chk==0 then return Duel.IsExistingMatchingCard(c99970023.thfilter,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,nil) end
  Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c99970023.thop(e,tp,eg,ep,ev,re,r,rp)
  if not e:GetHandler():IsRelateToEffect(e) then return end
  Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
  local g=Duel.SelectMatchingCard(tp,c99970023.thfilter,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,1,nil)
  if g:GetCount()>0 then

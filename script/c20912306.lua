...
function c20912306.initial_effect(c)
	...
	e2:SetTarget(c20912306.eqtg)
	e2:SetOperation(c20912306.eqop)
	c:RegisterEffect(e2)
	--destroy sub1
	...
	e4:SetValue(c20912306.repval2)
	c:RegisterEffect(e4)
	--equip2
	local e5=Effect.CreateEffect(c)
	e5:SetDescription(aux.Stringid(20912306,0))
	e5:SetType(EFFECT_TYPE_IGNITION)
	e5:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e5:SetCategory(CATEGORY_EQUIP)
	e5:SetRange(LOCATION_GRAVE)
	e5:SetCountLimit(1,20912306)
	e5:SetCost(c20912306.cost)
	e5:SetTarget(c20912306.target)
	e5:SetOperation(c20912306.operation)
	c:RegisterEffect(e5)
	--add setcode
...
function c20912306.repval2(e,re,r,rp)
	return bit.band(r,REASON_EFFECT)~=0
end
function c20912306.equfilter(c)
...
function c20912306.eqtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingTarget(c20912306.equfilter,tp,LOCATION_MZONE,0,1,e:GetHandler()) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	Duel.SelectTarget(tp,c20912306.equfilter,tp,LOCATION_MZONE,0,1,1,e:GetHandler())
	Duel.SetOperationInfo(0,CATEGORY_EQUIP,e:GetHandler(),1,0,0)
end
function c20912306.eqop(e,tp,eg,ep,ev,re,r,rp)
...
function c20912306.eqlimit(e,c)
	return c:IsRace(RACE_WARRIOR)
end
function c20912306.thacfilter(c)
	return c:IsSetCard(0xd0a2) and c:IsType(TYPE_EQUIP) and c:IsAbleToRemoveAsCost()
end
function c20912306.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c20912306.thacfilter,tp,LOCATION_GRAVE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,c20912306.thacfilter,tp,LOCATION_GRAVE,0,1,1,nil)
	Duel.Remove(g,POS_FACEUP,REASON_COST)
end
function c20912306.equipfilter(c)
	return c:IsFaceup() and c:IsSetCard(0xd0a2)
end
function c20912306.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c20912306.equipfilter(chkc) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingTarget(c20912306.equipfilter,tp,LOCATION_MZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	Duel.SelectTarget(tp,c20912306.equipfilter,tp,LOCATION_MZONE,0,1,1,nil)
end
function c20912306.operation(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	...
	e1:SetCode(EFFECT_EQUIP_LIMIT)
	e1:SetReset(RESET_EVENT+0x1fe0000)
	e1:SetValue(c20912306.eqlimit2)
	e1:SetLabelObject(tc)
	c:RegisterEffect(e1)
	--pierce
	local e2=Effect.CreateEffect(c)
...
function c20912306.eqlimit2(e,c)

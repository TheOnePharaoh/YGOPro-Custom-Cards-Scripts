...
function c87002899.initial_effect(c)
	...
	e1:SetCountLimit(1,87002899)
	e1:SetTarget(c87002899.spectg)
	e1:SetOperation(c87002899.specop)
	c:RegisterEffect(e1)
	local e2=e1:Clone()
	...
	e5:SetDescription(aux.Stringid(87002899,1))
	e5:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e5:SetCategory(CATEGORY_EQUIP)
	e5:SetType(EFFECT_TYPE_IGNITION)
	e5:SetRange(LOCATION_MZONE)
	e5:SetTarget(c87002899.eqtg)
	e5:SetOperation(c87002899.eqop)
	c:RegisterEffect(e5)
	--unequip
	local e6=Effect.CreateEffect(c)
	e6:SetDescription(aux.Stringid(87002899,2))
...
function c87002899.spectg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c87002899.spefilter,tp,LOCATION_DECK,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_DECK)
end
function c87002899.specop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c87002899.spefilter,tp,LOCATION_DECK,0,1,1,nil,e,tp)
...
function c87002899.eqtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g=Duel.SelectTarget(tp,c87002899.filter,tp,LOCATION_MZONE,0,1,1,e:GetHandler())
	Duel.SetOperationInfo(0,CATEGORY_EQUIP,g,1,0,0)
	e:GetHandler():RegisterFlagEffect(87002899,RESET_EVENT+0x7e0000+RESET_PHASE+PHASE_END,0,1)
end
function c87002899.eqop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local tc=Duel.GetFirstTarget()
	if not c:IsRelateToEffect(e) or c:IsFacedown() then return end
	if not tc:IsRelateToEffect(e) or not c87002899.filter(tc) then
...
function c87002899.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()

...
function c99970160.initial_effect(c)
  ...
  e1:SetCondition(c99970160.spcon)
  c:RegisterEffect(e1)
  --Gain ATK
...
function c99970160.spfilter1(c)
  return c:IsFaceup() and c:IsSetCard(9997) and c:IsType(TYPE_MONSTER) and not c:IsCode(99970160)
end
function c99970160.spcon(e,c)
  if c==nil then return true end
  return Duel.GetLocationCount(c:GetControler(),LOCATION_MZONE)>0 and
  Duel.IsExistingMatchingCard(c99970160.spfilter1,c:GetControler(),LOCATION_MZONE,0,1,nil)
end
...
function c99970160.atktg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
  ...
  Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(99970160,2))
...
function c99970160.spfilter2(c,e,tp)
...
function c99970160.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
  ...
  and Duel.IsExistingMatchingCard(c99970160.spfilter2,tp,LOCATION_DECK,0,1,nil,e,tp) end
  Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_DECK)
end
function c99970160.spop(e,tp,eg,ep,ev,re,r,rp)
  if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
  Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
  local g=Duel.SelectMatchingCard(tp,c99970160.spfilter2,tp,LOCATION_DECK,0,1,1,nil,e,tp)

--Duality
function c512000088.initial_effect(c)
	--Activate
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetTarget(c512000088.target)
	e1:SetOperation(c512000088.operation)
	c:RegisterEffect(e1)
end
function c512000088.filter1(c,e,tp)
	return c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c512000088.filter2(c,e,tp)
	return c:IsAttribute(ATTRIBUTE_DARK) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c512000088.filter3(c,e,tp)
	return c:IsCode(47297616) and c:IsCanBeSpecialSummoned(e,0,tp,true,false)
end
function c512000088.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local splight=Duel.IsExistingMatchingCard(c512000088.filter1,tp,LOCATION_HAND,0,1,nil,e,tp)
	local spdark=Duel.IsExistingMatchingCard(c512000088.filter2,tp,LOCATION_HAND,0,1,nil,e,tp)
	local ldd=Duel.IsExistingMatchingCard(c512000088.filter3,tp,LOCATION_HAND,0,1,nil,e,tp)
...
function c512000088.operation(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	if e:GetLabel()==0 then
		local g=Duel.SelectMatchingCard(tp,c512000088.filter2,tp,LOCATION_HAND,0,1,1,nil,e,tp)
		if g:GetCount()>0 then
			Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP)
		end
	elseif e:GetLabel()==1 then
		local g=Duel.SelectMatchingCard(tp,c512000088.filter1,tp,LOCATION_HAND,0,1,1,nil,e,tp)
		if g:GetCount()>0 then
			Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP)
		end
	else
		local g=Duel.SelectMatchingCard(tp,c512000088.filter3,tp,LOCATION_HAND,0,1,1,nil,e,tp)
		if g:GetCount()>0 then

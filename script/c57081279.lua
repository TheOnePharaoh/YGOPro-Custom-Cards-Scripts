...
function c57081279.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(57081279,0))
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e1:SetRange(LOCATION_MZONE)
	e1:SetCountLimit(1)
	e1:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e1:SetCondition(aux.IsDualState)
	e1:SetTarget(c57081279.target)
	e1:SetOperation(c57081279.operation)
	c:RegisterEffect(e1)
	--destroy
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_CONTINUOUS)
	e2:SetCode(EVENT_LEAVE_FIELD)
	e2:SetOperation(c57081279.desop)
	c:RegisterEffect(e2)
	--tuner related
...
function c57081279.desfilter(c,rc)
	return rc:GetCardTarget():IsContains(c) and c:IsRace(RACE_MACHINE)
end
function c57081279.desop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:GetCardTargetCount()>0 then
		local dg=Duel.GetMatchingGroup(c57081279.desfilter,tp,LOCATION_MZONE,LOCATION_MZONE,nil,c)

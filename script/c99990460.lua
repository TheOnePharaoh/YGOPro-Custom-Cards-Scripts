...
function c99990460.initial_effect(c)
  ...
  e1:SetCondition(c99990460.dmgcon)
  e1:SetTarget(c99990460.dmgtg)
  e1:SetOperation(c99990460.dmgop)
  c:RegisterEffect(e1)
  --Special Summon
  ...
  e4:SetCondition(c99990460.atkcon2)
  e4:SetOperation(c99990460.atkop)
  c:RegisterEffect(e4)
  ...
  e5:SetCondition(c99990460.atkcon3)
  e5:SetOperation(c99990460.atkop)
  c:RegisterEffect(e5)  
end
function c99990460.dmgcon(e,tp,eg,ep,ev,re,r,rp)
  return e:GetHandler():GetSummonType()==SUMMON_TYPE_SYNCHRO
end
function c99990460.dmgfilter(c)
  return c:IsFacedown()
end
function c99990460.dmgtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
  if chk==0 then return Duel.IsExistingTarget(c99990460.dmgfilter,tp,0,LOCATION_SZONE,1,nil) end
  local dmg=Duel.GetMatchingGroupCount(c99990460.dmgfilter,e:GetHandler():GetControler(),0,LOCATION_SZONE,nil)*500
  Duel.SetTargetPlayer(1-tp)
  Duel.SetOperationInfo(0,CATEGORY_DAMAGE,nil,0,1-tp,dmg)
end
function c99990460.dmgop(e,tp,eg,ep,ev,re,r,rp)
  local p=Duel.GetChainInfo(0,CHAININFO_TARGET_PLAYER)
  local dmg=Duel.GetMatchingGroupCount(c99990460.dmgfilter,e:GetHandler():GetControler(),0,LOCATION_SZONE,nil)*500
  Duel.Damage(p,dmg,REASON_EFFECT)
end
function c99990460.spfilter(c,e,tp)
...
function c99990460.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
  if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c99990460.spfilter(chkc,e,tp) end
  if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
  and Duel.IsExistingTarget(c99990460.spfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
  Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
  local g=Duel.SelectTarget(tp,c99990460.spfilter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
...
function c99990460.atkcon2(e,tp,eg,ep,ev,re,r,rp)
  local tc=eg:GetFirst()
  local bc=tc:GetBattleTarget()
...
function c99990460.atkcon3(e,tp,eg,ep,ev,re,r,rp)
  local tc=eg:GetFirst()
  local bc=tc:GetBattleTarget()
  if tc==nil then return false

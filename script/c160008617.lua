...
function c160008617.filter(c)
	return c:IsFaceup() and c:IsSetCard(0x11a) and c:IsCanAddCounter(0x1088,1)
end
function c160008617.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	Duel.SetOperationInfo(0,CATEGORY_COUNTER,nil,1,0,0x1088)
end

function c160008617.activate(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetFirstTarget()
	if tc and tc:IsFaceup() and tc:IsRelateToEffect(e) and tc:AddCounter(0x1088,1) then
		end
	end

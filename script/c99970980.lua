...
function c99970980.initial_effect(c)
  ...
  e1:SetTarget(c99970980.natg)
  e1:SetOperation(c99970980.naop)
  c:RegisterEffect(e1)
  --To Hand
...
function c99970980.natg(e,tp,eg,ep,ev,re,r,rp,chk)
  if chk==0 then return true end
end
function c99970980.naop(e,tp,eg,ep,ev,re,r,rp)
  local c=e:GetHandler()
  if Duel.NegateAttack() and c:IsRelateToEffect(e) then
  Duel.SendtoGrave(e:GetHandler(),REASON_EFFECT)
  if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
  local g=Duel.GetMatchingGroup(c99970980.spfilter,tp,LOCATION_HAND+LOCATION_DECK,0,nil,e,tp)
...
function c99970980.thop(e,tp,eg,ep,ev,re,r,rp)
  ...
  local g=Duel.SelectMatchingCard(tp,c99970980.thfilter,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,1,nil)
  if g:GetCount()>0 then

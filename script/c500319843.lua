...
function c500319843.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		local mg1=Duel.GetMatchingGroup(c500319843.filter1,tp,LOCATION_EXTRA+LOCATION_MZONE,0,nil,e)
		...
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(c500319843.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg2,mf,chkf)
			end
		end
		return res
	end
...
function c500319843.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if ce~=nil then
		...
		mg2=fgroup(ce,e,tp)
		local mf=ce:GetValue()
		sg2=Duel.GetMatchingGroup(c500319843.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg2,mf,chkf)
	end

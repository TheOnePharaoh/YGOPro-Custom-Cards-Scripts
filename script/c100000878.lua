...
function c100000878.filterrgr(c)
	return  c:IsSetCard(0x10E) and c:IsType(TYPE_MONSTER) and c:IsAbleToRemoveAsCost()
end
...
function c100000878.filter3(c)
	return c:IsAbleToRemove() and not c:IsCode(100000878)
end
function c100000878.target3(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c100000878.filter3,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) end
	local g=Duel.GetMatchingGroup(c100000878.filter3,tp,0,LOCATION_ONFIELD,nil)
	Duel.SetOperationInfo(0,CATEGORY_REMOVE,g,1,0,0)
end
function c100000878.activate3(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,c100000878.filter3,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil)

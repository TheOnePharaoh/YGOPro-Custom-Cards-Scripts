...
function c90000043.initial_effect(c)
	...
	e3:SetOperation(c90000043.operation2)
	c:RegisterEffect(e3)
end
...
function c90000043.filter(c,tp)
...
function c90000043.target2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return eg:IsExists(c90000043.filter,1,nil,tp) end
	return true
end
function c90000043.value(e,c)
	return c90000043.filter(c,e:GetHandlerPlayer())
end
function c90000043.filter2(c)
...
function c90000043.condition(e,tp,eg,ep,ev,re,r,rp)
	return tp==Duel.GetTurnPlayer() and Duel.IsExistingMatchingCard(c90000043.filter2,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil)
end
function c90000043.target3(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	local val=Duel.GetMatchingGroupCount(c90000043.filter2,tp,LOCATION_MZONE,LOCATION_MZONE,nil)*125

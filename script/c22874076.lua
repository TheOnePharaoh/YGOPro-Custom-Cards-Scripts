...
function c22874076.initial_effect(c)
	...
	e1:SetTarget(c22874076.target)
	e1:SetOperation(c22874076.operation)
	c:RegisterEffect(e1)
	--Equip limit
...
function c22874076.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and c22874076.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c22874076.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	Duel.SelectTarget(tp,c22874076.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_EQUIP,e:GetHandler(),1,0,0)
end
function c22874076.operation(e,tp,eg,ep,ev,re,r,rp)

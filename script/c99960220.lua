...
function c99960220.initial_effect(c)
  ...
  e2:SetCountLimit(1,99960220+EFFECT_COUNT_CODE_OATH)
  e2:SetRange(LOCATION_HAND)
  e2:SetCost(c99960220.thcost)
  e2:SetTarget(c99960220.thtg)
  e2:SetOperation(c99960220.thop)
  c:RegisterEffect(e2)
end
function c99960220.tdfilter1(c)
  return c:IsFaceup() and c:IsAbleToDeck()
end
function c99960220.tdfilter2(c)
  return c:IsFaceup() and c:IsSetCard(0x996) and c:IsAbleToDeck()
end
function c99960220.tdtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
  if chkc then return false end
  if chk==0 then return Duel.IsExistingTarget(c99960220.tdfilter1,tp,0,LOCATION_MZONE,1,nil)
  and Duel.IsExistingTarget(c99960220.tdfilter2,tp,LOCATION_MZONE,0,1,nil) end
  Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
  local g1=Duel.SelectTarget(tp,c99960220.tdfilter1,tp,0,LOCATION_MZONE,1,1,nil)
  Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
  local g2=Duel.SelectTarget(tp,c99960220.tdfilter2,tp,LOCATION_MZONE,0,1,1,nil)
  g1:Merge(g2)
...
function c99960220.tdop(e,tp,eg,ep,ev,re,r,rp)
  ...
  local tc=g:GetFirst()
  if (tc:IsCode(99960180) or tc:IsCode(99960200) or tc:IsCode(99960240) or tc:IsCode(99960260)) and tc:IsAbleToHand() then
...
function c99960220.thcost(e,tp,eg,ep,ev,re,r,rp,chk)

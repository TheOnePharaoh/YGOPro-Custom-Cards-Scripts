...
function c20912274.initial_effect(c)
	...
	e1:SetTarget(c20912274.target)
	e1:SetOperation(c20912274.operation)
	c:RegisterEffect(e1)
	--Atk up
...
function c20912274.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and c20912274.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c20912274.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	Duel.SelectTarget(tp,c20912274.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil)

...
function scard.d_tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.GetFieldGroupCount(tp,LOCATION_DECK,0)>0 and Duel.IsExistingTarget(Card.IsFaceup,tp,LOCATION_MZONE,0,1,nil) end
...
function scard.d_op(e,tp,eg,ep,ev,re,r,rp)
	...
	if ct>0 then
		local n=1
		if ct>1 then
			Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(s_id,8))
			if ct==2 then
				n=Duel.AnnounceNumber(tp,1,2)
			else
				n=Duel.AnnounceNumber(tp,1,2,3)
			end
		end
		...
		local sg=g:FilterSelect(tp,scard.d_fil,1,1,nil)
		...
		if ct>1 then
			local c=e:GetHandler()
			if c:CheckRemoveOverlayCard(tp,1,REASON_COST) and Duel.SelectYesNo(tp,aux.Stringid(s_id,9)) then
				c:RemoveOverlayCard(tp,1,1,REASON_COST)
				if n>1 then	Duel.SortDecktop(tp,tp,n) end
			else
				Duel.ShuffleDeck(tp)
			end
		end
	...
	if tc and tc:IsRelateToEffect(e) and tc:IsFaceup() then
		tc:ResetEffect(EFFECT_CHANGE_CODE,RESET_CODE)

function c14981505.initial_effect(c)
	aux.AddSynchroProcedure(c,aux.FilterBoolFunction(Card.IsRace,0x100000),aux.NonTuner(nil),1)
	c:EnableReviveLimit()
		local e1=Effect.CreateEffect(c)
		e1:SetDescription(aux.Stringid(14981505,0))
		e1:SetCategory(CATEGORY_TOGRAVE+CATEGORY_DESTROY)
		e1:SetType(EFFECT_TYPE_IGNITION)
		e1:SetProperty(EFFECT_FLAG_CARD_TARGET)
		e1:SetCountLimit(1)
		e1:SetRange(LOCATION_MZONE)
		e1:SetCost(c14981505.cost)
		e1:SetTarget(c14981505.destg)
		e1:SetOperation(c14981505.desop)
		c:RegisterEffect(e1)
	end
function c14981505.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckLPCost(tp,1000) end
	Duel.PayLPCost(tp,1000)
end	
function c14981505.tgfilter(c)
...
function c14981505.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if g1:GetFirst():IsRelateToEffect(e) and Duel.Destroy(g1,REASON_EFFECT)~=0 then
		local hg=g2:Filter(c14981505.tgfilter,nil,e)

...
function c99960000.initial_effect(c)
 ...
 --Attach
  ...
  e1:SetCondition(c99960000.matcon)
  e1:SetTarget(c99960000.mattg)
  e1:SetOperation(c99960000.matop)
  c:RegisterEffect(e1)
...
function c99960000.matcon(e,tp,eg,ep,ev,re,r,rp)
  return e:GetHandler():GetSummonType()==SUMMON_TYPE_SPECIAL+9996
end
function c99960000.matfilter(c)
  return c:IsType(TYPE_MONSTER+TYPE_SPELL+TYPE_TRAP) and not c:IsHasEffect(EFFECT_NECRO_VALLEY)
end
function c99960000.mattg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
  if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c99960000.matfilter(chkc) end
  if chk==0 then return Duel.IsExistingTarget(c99960000.matfilter,tp,LOCATION_GRAVE,0,1,nil) end
  Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(99960000,2))
  local g=Duel.SelectTarget(tp,c99960000.matfilter,tp,LOCATION_GRAVE,0,1,1,nil)
  Duel.SetOperationInfo(0,CATEGORY_LEAVE_GRAVE,g,1,0,0)
end
function c99960000.matop(e,tp,eg,ep,ev,re,r,rp)
...
function c99960000.fildfilter(c)
  return c:IsFaceup() and c:IsCode(99960300)
end
function c99960000.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
  if chk==0 then return (Duel.GetLP(tp)>=250 and Duel.IsExistingMatchingCard(c99960000.fildfilter,tp,LOCATION_ONFIELD,0,1,nil))
  or (Duel.GetLP(tp)>=500 and not Duel.IsExistingMatchingCard(c99960000.fildfilter,tp,LOCATION_ONFIELD,0,1,nil)) end
  if Duel.IsExistingMatchingCard(c99960000.fildfilter,tp,LOCATION_ONFIELD,0,1,nil) then 
  Duel.PayLPCost(tp,250)
  elseif not Duel.IsExistingMatchingCard(c99960000.fildfilter,tp,LOCATION_ONFIELD,0,1,nil) then
...
function c99960000.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)

...
function c99970500.initial_effect(c)
  ...
  e1:SetTarget(c99970500.natg)
  e1:SetOperation(c99970500.naop)
  c:RegisterEffect(e1)
  --To Hand
...
function c99970500.natg(e,tp,eg,ep,ev,re,r,rp,chk)
  if chk==0 then return true end
end
function c99970500.naop(e,tp,eg,ep,ev,re,r,rp)
  local c=e:GetHandler()
  if Duel.NegateAttack() and c:IsRelateToEffect(e) then
  Duel.SendtoGrave(e:GetHandler(),REASON_EFFECT)
  if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
  local g=Duel.GetMatchingGroup(c99970500.spfilter,tp,LOCATION_HAND+LOCATION_DECK,0,nil,e,tp)
...
function c99970500.thop(e,tp,eg,ep,ev,re,r,rp)
  ...
  local g=Duel.SelectMatchingCard(tp,c99970500.thfilter,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,1,nil)
  if g:GetCount()>0 then

...
function c99990100.initial_effect(c)
	...
	e2:SetOperation(c99990100.atkop2)
	c:RegisterEffect(e2)
	--ATK/DEF
	...
	e3:SetCondition(c99990100.atkcon)
	e3:SetOperation(c99990100.atkop)
	c:RegisterEffect(e3)
	...
	e4:SetCondition(c99990100.atkcon2)
	...
	e5:SetCondition(c99990100.atkcon3)
	e5:SetOperation(c99990100.atkop)
	c:RegisterEffect(e5)
end
function c99990100.filter1(c,e,tp)
...
function c99990100.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_DECK+LOCATION_HAND) and c99990100.filter1(chkc,e,tp) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsExistingTarget(c99990100.filter1,tp,LOCATION_DECK+LOCATION_HAND,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,c99990100.filter1,tp,LOCATION_DECK+LOCATION_HAND,0,1,1,nil,e,tp)
...
function c99990100.atkcon3(e,tp,eg,ep,ev,re,r,rp)
	local tc=eg:GetFirst()
	local bc=tc:GetBattleTarget()
	if tc==nil then return false

--Mecha Ojama King
function c512000097.initial_effect(c)
	c:EnableReviveLimit()
	--spsummon limit
	...
	e2:SetTarget(c512000097.sptg)
	e2:SetOperation(c512000097.spop)
	c:RegisterEffect(e2)
end
function c512000097.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c512000097.spop(e,tp,eg,ep,ev,re,r,rp)
	if e:GetLabel()==0 then
		...
		for i=1,3 do
			...
			if Duel.SpecialSummonStep(token,0,tp,tp,false,false,POS_FACEUP_DEFENSE) then
				...
				e2:SetOperation(c512000097.damop2)
		...
		for i=1,3 do
			...
			if Duel.SpecialSummonStep(token,0,tp,1-tp,false,false,POS_FACEUP_DEFENSE) then
				...
				e2:SetCode(EVENT_DESTROY)
				e2:SetProperty(EFFECT_FLAG_CANNOT_DISABLE)
				e2:SetOperation(c512000097.damop)
				e2:SetReset(RESET_EVENT+0x1fe0000)
				token:RegisterEffect(e2,true)
				local e3=e1:Clone()
...
function c512000097.damop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Damage(tp,500,REASON_EFFECT)
end
function c512000097.damop2(e,tp,eg,ep,ev,re,r,rp)

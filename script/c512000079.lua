--Takeover
function c512000079.initial_effect(c)
	--Activate
	...
	e1:SetTarget(c512000079.target)
	e1:SetOperation(c512000079.activate)
	c:RegisterEffect(e1)
end
function c512000079.filter(c)
	return c:IsFaceup() and c:GetCounter(0)~=0 and c:IsDestructable()
end
function c512000079.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsOnField() and c512000079.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c512000079.filter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,c512000079.filter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,g,1,0,0)
	Duel.SetOperationInfo(0,CATEGORY_RECOVER,nil,0,tp,g:GetFirst():GetCounter(0)*100)
end
function c512000079.activate(e,tp,eg,ep,ev,re,r,rp)

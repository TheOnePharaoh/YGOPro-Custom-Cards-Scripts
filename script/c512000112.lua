--Graveyard Rebound
function c512000112.initial_effect(c)
	--to deck
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(511001629,0))
	e1:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_F)
	e1:SetCode(EVENT_TO_GRAVE)
	e1:SetCondition(c512000112.con)
	e1:SetOperation(c512000112.op)
	c:RegisterEffect(e1)
end
function c512000112.con(e,tp,eg,ep,ev,re,r,rp)
...
function c512000112.op(e,tp,eg,ep,ev,re,r,rp)
	local e1=Effect.CreateEffect(e:GetHandler())
	e1:SetCategory(CATEGORY_TODECK)
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetRange(LOCATION_GRAVE)
	e1:SetTarget(c512000112.tdtg)
	e1:SetOperation(c512000112.tdop)
	if Duel.GetTurnPlayer()==tp then
		e1:SetLabel(Duel.GetTurnCount())
		e1:SetCondition(c512000112.tdcon)
...
function c512000112.tdcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetTurnCount()~=e:GetLabel()
end
function c512000112.tdtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsAbleToDeck() end
	Duel.SetOperationInfo(0,CATEGORY_TODECK,e:GetHandler(),1,0,0)
end
function c512000112.tdop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) then
		Duel.SendtoDeck(c,nil,2,REASON_EFFECT)
		Duel.RegisterFlagEffect(tp,512000112,RESET_PHASE+PHASE_END,0,1)
		local e1=Effect.CreateEffect(c)
		e1:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
		e1:SetCode(EVENT_ADJUST)
		e1:SetOperation(c512000112.operation)
		e1:SetReset(RESET_PHASE+PHASE_END)
		Duel.RegisterEffect(e1,tp)
	end
end
function c512000112.operation(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()	
	...
	while tc do
		...
		if tc:GetFlagEffect(512000112)==0 and te then
			...
			e1:SetRange(LOCATION_GRAVE)
			e1:SetCondition(c512000112.accon)
			if te:GetCost() then
				e1:SetCost(te:GetCost())
			end
			e1:SetTarget(c512000112.actg)
			e1:SetOperation(c512000112.acop)
			...
			tc:RegisterFlagEffect(512000112,RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END,0,1)
		end
		tc=g:GetNext()
	end
end
function c512000112.accon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local te=c:GetActivateEffect()
	local condition=te:GetCondition()
	return (not condition or condition(te,tp,eg,ep,ev,re,r,rp)) 
		and Duel.GetFlagEffect(tp,512000112)>0
		and not e:GetHandler():IsStatus(STATUS_CHAINING)
end
function c512000112.actg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	local te=c:GetActivateEffect()
...
function c512000112.acop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local te=c:GetActivateEffect()

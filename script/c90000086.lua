...
function c90000086.initial_effect(c)
	...
	e3:SetCondition(c90000086.condition)
	e3:SetOperation(c90000086.operation)
	c:RegisterEffect(e3)
	--Activate Limit
	...
	e4:SetOperation(c90000086.operation2)
	c:RegisterEffect(e4)
	--Damage
	...
	e5:SetCondition(c90000086.condition3)
	e5:SetTarget(c90000086.target)
	e5:SetOperation(c90000086.operation3)
	c:RegisterEffect(e5)
end
function c90000086.fil(c)
	return c:IsFaceup() and c:IsSetCard(0x4d)
end
function c90000086.tg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingTarget(c90000086.fil,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	Duel.SelectTarget(tp,c90000086.fil,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_EQUIP,e:GetHandler(),1,0,0)
end
function c90000086.op(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetFirstTarget()
	if e:GetHandler():IsRelateToEffect(e) and tc:IsRelateToEffect(e) and tc:IsFaceup() then
		Duel.Equip(tp,e:GetHandler(),tc)
	end
end
function c90000086.val(e,c)
...
function c90000086.operation(e,tp,eg,ep,ev,re,r,rp)
	Duel.SetChainLimit(c90000086.limit)
end
function c90000086.limit(e,ep,tp)
	return ep==tp
end
...
function c90000086.value(e,re,tp)
	return not re:GetHandler():GetEquipTarget():IsImmuneToEffect(e)
end
function c90000086.condition3(e,tp,eg,ep,ev,re,r,rp)
...
function c90000086.operation3(e,tp,eg,ep,ev,re,r,rp)

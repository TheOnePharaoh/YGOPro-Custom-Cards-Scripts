...
function c99990440.initial_effect(c)
	...
	e4:SetCondition(c99990440.atkcon2)
	e4:SetOperation(c99990440.atkop)
	c:RegisterEffect(e4)
	...
	e5:SetCondition(c99990440.atkcon3)
	e5:SetOperation(c99990440.atkop)
	c:RegisterEffect(e5)
	local e6=Effect.CreateEffect(c)
	e6:SetCategory(CATEGORY_ATKCHANGE+CATEGORY_DEFCHANGE)
	e6:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_F)
	e6:SetCode(EVENT_SPSUMMON_SUCCESS)
	e6:SetCondition(c99990440.atkcon4)
	e6:SetOperation(c99990440.atkop2)
	c:RegisterEffect(e6)
end
...
function c99990440.atkcon2(e,tp,eg,ep,ev,re,r,rp)
	local tc=eg:GetFirst()
	local bc=tc:GetBattleTarget()
...
function c99990440.atkcon3(e,tp,eg,ep,ev,re,r,rp)
	local tc=eg:GetFirst()
	local bc=tc:GetBattleTarget()
	if tc==nil then return false
...
function c99990440.filter1(c)
	return c:IsFaceup() and c:IsSetCard(9999) and c:IsType(TYPE_MONSTER)
end
function c99990440.atkcon4(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetFieldGroupCount(tp,0,LOCATION_MZONE)>0 and e:GetHandler():GetSummonType()==SUMMON_TYPE_SYNCHRO
end
function c99990440.atkop2(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local tg=Duel.GetMatchingGroup(Card.IsFaceup,tp,0,LOCATION_MZONE,nil)
	local tc=tg:GetFirst()
	local val=Duel.GetMatchingGroupCount(c99990440.filter1,e:GetOwnerPlayer(),LOCATION_MZONE,0,nil)*-200
	while tc do
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_SINGLE)
	e1:SetCode(EFFECT_UPDATE_ATTACK)
